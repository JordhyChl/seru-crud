...
class DetailTokoActivity : AppCompatActivity(),
    DetailTokoContract.View,
    OnMapReadyCallback,
    EasyPermissions.PermissionCallbacks{

    companion object {
        val TAG = DetailTokoActivity::class.java.simpleName
        private const val LOCATION_PERMISSION_REQUEST_CODE = 1
    }

    @Inject lateinit var presenter: DetailTokoContract.Presenter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val idToko = intent.getStringExtra("IDTOKO")
        Log.i(TAG, "value id toko : $idToko")
    }
    ...
    private fun showViewData(res: DetailTokoRes){
        ...
        if (res.outlet_status != null) {
            ...
            initTimelineView()
            tvStatusValidasi.text = res.outlet_status.status.status_value

            val jadwalKirim = res.outlet_status.dateRecommendation
            val date = SimpleDateFormat("dd - MMM - yyyy", Locale("in")).format(jadwalKirim * 1000)
            Log.d(TAG, "date kirim long : $jadwalKirim")
            tvJadwalKirim.text = date
            tvCatatan.text = res.outlet_status.status.note_outlet
        }else{
